...
import com.daolab.daolabplayer.api.ovp.services.StatsService;
...
public class DaolabStatsPlugin extends PKPlugin {
    private static final PKLog log = PKLog.get("DaolabStatsPlugin");

    private Context context;
    private AdInfo adInfo;
    private Player player;
    private PKMediaConfig mediaConfig;
    private DaolabStatsConfig pluginConfig;
    ...
    private static DaolabStatsConfig parseConfig(Object config) {
        if (config instanceof DaolabStatsConfig) {
            return ((DaolabStatsConfig) config);

        } else if (config instanceof JsonObject) {
            return new Gson().fromJson(((JsonObject) config), DaolabStatsConfig.class);
        }
        return null;
    }
            ...
            if (event instanceof PlayerEvent) {
                ...
                switch (((PlayerEvent) event).type) {
                    case METADATA_AVAILABLE:
                        ...
                        break;
                    case STATE_CHANGED:
                        DaolabStatsPlugin.this.onEvent((PlayerEvent.StateChanged) event);
                        break;
                    case ERROR:
                ...
                DaolabStatsPlugin.this.onEvent((AdEvent) event);
            }
        }
    };
    ...
    public void onEvent(AdEvent event) {
        ...
        messageBus.post(new DaolabStatsEvent.DaolabStatsReport(event.eventType().toString()));
    }
    ...
    /**
     * Send stats event to Daolab stats DB
     *
     * @param eventType - Enum stating Daolab state events
     */
    private void sendAnalyticsEvent(final KStatsEvent eventType) {
        ...
        requestBuilder.completion(new OnRequestCompletion() {
            ...
            public void onComplete(ResponseElement response) {
                ...
                messageBus.post(new DaolabStatsEvent.DaolabStatsReport(eventType.toString()));
                if (hasSeeked) {
                    hasSeeked = false;
                }
            }
